...
public class VehicleService {
    ...
    public Vehicle createVehicle(@NotNull VehicleRequest request){
        String plate = request.getLicensePlate().toUpperCase();
        long customerId = request.getCustomerId();

        if(vehicleRepository.existsByLicensePlate(plate)){
            throw new IllegalArgumentException("Já existe um veículo com placa: " + plate);
        }
        Customer customer = customerRepository.findById(customerId)
                .orElseThrow(() -> new IllegalArgumentException("Cliente não encontrado com documento: " + customerId));
    ...
    public Vehicle updateVehicle(@NotNull Long id, @NotNull VehicleRequest request) {
        ...
        Customer custome = existingVehicle.getCustomer();
        if(!existingVehicle.getCustomer().getId().equals(request.getCustomerId())){
            custome =  customerRepository.findById(request.getCustomerId()).orElseThrow(()
                    -> new IllegalArgumentException("Clente não encontrado com ID: " + request.getCustomerId()));
        }
        ...
        existingVehicle.setCustomer(custome);
